...
public class Cart {
    ...
    public void addProduct(Product product) {
        getCartItemByItem(product).ifPresentOrElse(CartItem::increaseCounter, () -> cartItems.add(new CartItem(product))
        ); recalculatePriceAndCounter();
    }
     public void decreaseProduct(Product product) {
        Optional<CartItem> oCartItem = getCartItemByItem(product);
        if (oCartItem.isPresent()) {CartItem cartItem = oCartItem.get();
    ...
    private void recalculatePriceAndCounter(){
        sum = cartItems.stream().map(CartItem::getPrice).reduce((double) 0, Double::sum);
        counter = cartItems.stream().mapToInt(CartItem::getCounter).reduce(0, (a,b) -> a + b );
    ...
    private Optional<CartItem> getCartItemByItem(Product product){
        return cartItems.stream().filter(i -> i.isEqual(product)).findFirst();
    }
